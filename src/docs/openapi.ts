import { getAbsensiOpenApiDocument } from "../app/absensi/route/absensi.route";
import { getDisseminationOpenApiDocument } from "../app/dissemination/route/dissemination.route";
import { getDisseminationDetailOpenApiDocument } from "../app/dissemination_detail/route/dissemination-detail.route";
import { getGradeOpenApiDocument } from "../app/grade/route/grade.route";
import { getMenuOpenApiDocument } from "../app/menu/route/menu.route";
import { getPositionOpenApiDocument } from "../app/position/route/position.route";
...
function createBaseDocument(baseUrl: string): OpenApiDocument {
  return {
    openapi: "3.0.3",
    info: {
      title: "API Diseminasi",
      version: "1.0.0",
      description: [
        "OpenAPI reference untuk backend Diseminasi berbasis Hono.",
        "",
        ...
        "- Upload memakai flow signed upload ke Cloudinary melalui `POST /api/uploads/signature`",
      ].join("\n"),
    },
    servers: [
      {
    ...
    tags: [
      ...
      { name: "Grades", description: "Grade master data" },
      { name: "Positions", description: "Position master data" },
      {
      ...
      {
        name: "Disseminations",
        description: "Dissemination header records",
      },
      {
        name: "Dissemination Details",
        description: "Dissemination detail records and evidences",
      },
      { name: "Absensis", description: "Attendance or evidence records" },
      {
        name: "Uploads",
                ...
                "application/json": {
                  schema: {
                    ...
                    properties: {
                      ...
                      message: {
                        type: "string",
                        example: "Welcome to API Diseminasi",
                      },
                      version: {
                        type: "string",
                        example: "1.0.0",
...
export function createOpenApiDocument(baseUrl: string) {
  const moduleDocuments = [
    ...
    mountOpenApiPaths(getGradeOpenApiDocument(baseUrl), "/api/grades"),
    mountOpenApiPaths(getPositionOpenApiDocument(baseUrl), "/api/positions"),
    mountOpenApiPaths(
      getRolePermissionOpenApiDocument(baseUrl),
      "/api/role-permissions",
    ),
    mountOpenApiPaths(
      getDisseminationOpenApiDocument(baseUrl),
      "/api/disseminations",
    ),
    mountOpenApiPaths(
      getDisseminationDetailOpenApiDocument(baseUrl),
      "/api/dissemination-details",
    ),
    mountOpenApiPaths(getAbsensiOpenApiDocument(baseUrl), "/api/absensis"),
...
<html lang="en">
  <head>
    ...
    <title>API Diseminasi Reference</title>
    <style>
      body {
        margin: 0;
      }
